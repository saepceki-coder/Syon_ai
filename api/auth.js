...
// Client ID Google (public, boleh hardcode — tapi lebih baik env)
...
async function ensureSchema() {
  ...
  // Migration: kalau kolom lama masih TIMESTAMP, ubah ke TIMESTAMPTZ
  try {
    ...
    await sql`ALTER TABLE otps ALTER COLUMN created_at TYPE TIMESTAMPTZ`;
    await sql`ALTER TABLE sessions ALTER COLUMN expires_at TYPE TIMESTAMPTZ`;
    await sql`ALTER TABLE sessions ALTER COLUMN created_at TYPE TIMESTAMPTZ`;
  } catch (e) {
    // ignore — kalau udah TIMESTAMPTZ atau error lain
  }
  schemaReady = true;
}
...
async function checkRateLimit(key, max, windowSec) {
  try {
    ...
    return true; // fail-open biar user nggak ke-lock karena bug
  }
}
...
async function handleRegister(req, res) {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(em) || em.length < 5 || em.length > 254)
    return res.status(400).json({ error: 'Email tidak valid' });
  if (String(password).length < 6) return res.status(400).json({ error: 'Password minimal 6 karakter' });
  if (String(password).length > 200) return res.status(400).json({ error: 'Password terlalu panjang' });
  if (String(name).trim().length < 1 || String(name).length > 60)
  ...
  if (existing.length) {
    // Re-register akun unverified — timpa nama+password, invalidation OTP lama
  ...
  try {
    await sendEmail(em, 'Kode Verifikasi Syon AI',
      emailTemplate(code, 'Verifikasi Email', 'Masukkan kode berikut untuk mengaktifkan akun Anda.'));
  } catch (e) {
...
async function handleForgot(req, res) {
  ...
  if (!users.length) {
    // Tetap return ok biar nggak bocorin email mana yang terdaftar
  ...
  try {
    await sendEmail(em, 'Reset Password Syon AI',
      emailTemplate(code, 'Reset Password', 'Masukkan kode berikut untuk mengganti password Anda.'));
  } catch (e) {
    return res.status(500).json({ error: 'Gagal kirim email: ' + e.message });
  }
...
async function handleReset(req, res) {
  ...
  if (String(newPassword).length > 200) return res.status(400).json({ error: 'Password terlalu panjang' });
  const em = sanitizeEmail(email);
  ...
  // Invalidate semua session lama (logout paksa semua device)
...
async function handleGoogleSync(req, res) {
  ...
  // 🔒 VERIFIKASI JWT GOOGLE — titik keamanan utama
  let payload;
  try {
  ...
  if (!payload || !payload.email || !payload.sub) {
    return res.status(401).json({ error: 'Data Google tidak lengkap' });
  }
  ...
  let users = await sql`SELECT id, email, name, google_id FROM users WHERE email = ${em}`;
  let userId;

  if (users.length) {
    userId = users[0].id;
    // Cegah account takeover: kalau user udah punya google_id berbeda, tolak
    ...
    const inserted = await sql`
      INSERT INTO users (email, name, google_id, verified) 
      VALUES (${em}, ${name}, ${googleId}, TRUE) 
      RETURNING id
    `;
